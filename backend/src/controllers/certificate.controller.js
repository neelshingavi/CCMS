...
exports.issueCertificate = async (req, res) => {
    try {
        ...
        // 6. Mint ASA on blockchain (if contract deployed)
        let assetId = null;
        let txnId = null;

        if (event.certificateAssetId) {
            try {
                // In production: use inner transactions to transfer pre-minted ASA
                // const result = await AlgorandClient.transferCertificate(event.certificateAssetId, user.wallet_address);
                // assetId = event.certificateAssetId;
                // txnId = result.txnId;
                logger.info(`Would mint certificate for ${user.wallet_address}`);
            } catch (blockchainError) {
                logger.error('Blockchain certificate mint failed:', blockchainError);
            }
        }
